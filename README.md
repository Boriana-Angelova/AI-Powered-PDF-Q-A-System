...
## 🛠️ How It Works
1. **PDF Parsing** — the document is read and split into text segments  
2. **Embedding Generation** — each segment is converted into a vector  
3. **ChromaDB Indexing** — vectors are stored for fast semantic search  
4. **User Query Processing** — the model extracts:  
   - the question  
   - the desired answer format (text / image / audio)  
5. **RAG Retrieval** — retrieves the most relevant context chunks  
6. **Answer Generation** — the LLM produces the final answer  
